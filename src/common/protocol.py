...
Recepção sempre via MessageReader, exclusivamente pelo método
MessageReader.recv_message. O reader mantém um buffer interno entre
...
Mesmo em conexões one-shot (e.g. flooding
...
def recv_exact(sock: socket.socket, n: int) -> bytes:
    ...
    Não usa o buffer interno do MessageReader; útil para casos onde
    não há buffer (ex.: testes, leituras isoladas). Em conexões persistentes,
    prefira MessageReader.recv_chunk.
...
def _decode_json_line(line: bytes) -> dict[str, Any]:
    """Decodifica uma linha como JSON UTF-8 ou levanta ProtocolError."""
    try:
