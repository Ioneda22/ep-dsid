...
def cliente(tmp_path: Path) -> Iterator[httpx.Client]:
    ...
    with httpx.Client(base_url=f"http://127.0.0.1:{porta}", timeout=5.0) as http_client:
        ...
        # Só se pode adicionar à playlist um hash conhecido no índice: registra
        # os arquivos de teste (upload) antes.
        for i, hash_arquivo in enumerate((HASH_A, HASH_B, HASH_C)):
            http_client.post(
                "/files/register",
                json={
                    "type": "REGISTER_FILE",
...
def test_adicionar_hash_malformado_retorna_400(cliente: httpx.Client) -> None:
    # Bug: a playlist aceitava qualquer string. Agora hash fora do formato
    # sha256 (64 hex) é rejeitado com INVALID_HASH (HTTP 400).
    pid = _criar(cliente)
    resposta = _adicionar(cliente, pid, "banana")
    assert resposta.status_code == 400
    assert resposta.json()["code"] == "INVALID_HASH"


def test_adicionar_hash_desconhecido_retorna_404(cliente: httpx.Client) -> None:
    # Hash com formato válido mas que não é de nenhum arquivo registrado.
    pid = _criar(cliente)
    resposta = _adicionar(cliente, pid, "dd" * 32)
    assert resposta.status_code == 404
    assert resposta.json()["code"] == "NOT_FOUND"
