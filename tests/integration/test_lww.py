...
def test_timestamp_menor_perde(indice: Index) -> None:
    ...
    tombstone_atrasado = _entry(ativo=False)
    assert not indice.apply_sync_entry(tombstone_atrasado, "tracker-2", timestamp=999.0)
...
def test_timestamp_maior_vence(indice: Index) -> None:
    _com_fonte_local(indice)  # versão local: t=1000
    tombstone_recente = _entry(ativo=False)
    assert indice.apply_sync_entry(tombstone_recente, "tracker-2", timestamp=1001.0)
...
def test_atualizacao_de_endereco_com_timestamp_maior(indice: Index) -> None:
    ...
    assert indice.apply_sync_entry(nova_versao, "tracker-2", timestamp=2000.0)
...
def test_empate_vence_maior_tracker_id_independente_da_ordem(
    ...
    for origem in ordem:
        indice.apply_sync_entry(versoes[origem], origem, timestamp=500.0)
...
def test_empate_com_escrita_local_compara_origens(indice: Index) -> None:
    ...
    assert not indice.apply_sync_entry(
        _entry(ip="10.0.0.0"), "tracker-0", timestamp=1000.0
    )
    # tracker-2 > tracker-1: empate vence, sobrescreve.
    assert indice.apply_sync_entry(_entry(ip="10.0.0.2"), "tracker-2", timestamp=1000.0)
    assert indice.get_peers_for_hash(HASH)[0].ip == "10.0.0.2"


def test_replay_identico_e_descartado(indice: Index) -> None:
    entry = _entry()
    assert indice.apply_sync_entry(entry, "tracker-2", timestamp=500.0)
    # Mesmo timestamp e mesma origem: empate não-estrito, descartado.
    assert not indice.apply_sync_entry(entry, "tracker-2", timestamp=500.0)
...
def test_registro_mais_recente_remove_tombstone(indice: Index) -> None:
    indice.apply_sync_entry(_entry(ativo=False), "tracker-2", timestamp=1000.0)
    assert indice.apply_sync_entry(_entry(), "tracker-3", timestamp=1001.0)
...
def test_registro_atrasado_nao_ressuscita_removido(indice: Index) -> None:
    ...
    indice.apply_sync_entry(_entry(ativo=False), "tracker-2", timestamp=1000.0)
    assert not indice.apply_sync_entry(_entry(), "tracker-3", timestamp=999.0)
    snapshot = indice.get_snapshot()
