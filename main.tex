...
\paragraph{Tracker}
\begin{itemize}
    \item Agrega um cluster de peers comuns. Mantém duas estruturas de índice:
    \begin{itemize}
...
% ============================================================
\textcolor{red}{Verificar se os indices estão sendo descritos da forma correta e na sua totalidade se todos os indices implementados no codigo estão sendo descritos aqui}
\subsection{Formato}
...
\textcolor{red}{Verificar a validade e a utilidade do SEARCH\_FORWARD já que os indices já são replicados}
...
\textcolor{red}{Verificar se o fluxo está correto ou se algumas etapas do fluxo estão faltantes. Verificar também se as mensagens estão com os parametros corretos, se as atualizações de tabela estão completas, etc. Além disso, caso seja necessário, adicionar algum fluxo importante faltante.}

Definidos os tipos de mensagem, descrevemos abaixo os três fluxos centrais do sistema.
...
\begin{enumerate}
    ...
    \item Tracker insere o peer em sua tabela \texttt{nome\_peer $\rightarrow$ ip:porta}, nenhum arquivo é registrado aqui.
\end{enumerate}
...
\textcolor{red}{Verificar essa sessão}
...
\textcolor{red}{Verificar essa parte -> a sequencia de eventos aqui pode ser mais detalhada (por meio das mensagens enviadas)}
...
Isso evita o seguinte problema: se um tracker estava desconectado e perdeu a mensagem de remoção, ele continuaria servindo um peer que já saiu. Com o tombstone, ao voltar à rede o tracker aplica o LWW normalmente e efetiva a remoção. Passado o período de retenção, o registro é descartado de vez.

\subsubsection*{Distribuição das cópias}

\textcolor{red}{Verificar essa seção}

O conjunto de trackers é \textbf{estático} (definido na configuração da rede), mas admite entrada dinâmica de novos trackers em tempo de execução via \textit{bootstrap node}. Já as réplicas dos arquivos são \textbf{dinâmicas e orientadas a demanda}: uma nova cópia surge exatamente quando e onde um peer baixa o arquivo.
...
\begin{itemize}
    ...
    \item \textbf{Arbitrária / bizantina:} \textbf{fora do escopo}. Não tratamos nós maliciosos ou com comportamento arbitrário; não há votação nem assinatura para detectar mentiras.
\end{itemize}
