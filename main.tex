...
\begin{itemize}
    ...
    \item \textbf{Eficiência de busca:} weak peers não precisam manter tabelas de roteamento complexas, pois delegam a busca ao tracker ao qual estão conectados.
...
Definida a arquitetura, a seguir iremos descrever os dois tipos de componente e como o conjunto de trackers se organiza.
...
\begin{itemize}
    ...
    \item Ao fazer upload de uma música, notifica o tracker com \texttt{metadados}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{nome\_da\_musica} $\rightarrow$ \texttt{conjunto de hashes} (permitimos músicas com o mesmo nome)
...
\begin{itemize}
    \item Os trackers formam uma rede totalmente conectada, em que cada tracker mantém uma lista de todos os outros trackers ativos.
...
\begin{itemize}
    ...
    \item \textbf{Camada de lógica de negócio:} gerenciamento de playlists, lookup nas tabelas de índice e roteamento de buscas.
...
\begin{figure}[H]
    ...
    \caption{Arquitetura em camadas do tracker (super peer), de forma a executar como processo único.}
...
Tanenbaum (Cap.~2.2) define \textit{middleware} como a camada que abstrai a heterogeneidade da rede, de forma a permitir que componentes distribuídos se comuniquem sem conhecer os detalhes uns dos outros. No PeerSpot, o middleware se apresenta entre os peers e entre um peer e seu tracker correspondente.

\subsection{Entre peer e tracker}
Um framework de comunicação padronizado, REST sobre HTTP, abstrai os detalhes de rede e permite que peers em plataformas diferentes se comuniquem com o tracker de forma uniforme. Como middleware nessa camada, foi utilizado o FastAPI (Python), que opera em conjunto com o servidor Uvicorn.

\subsection{Entre peers}
O protocolo de troca de chunks sobre TCP padroniza como dois peers negociam quais partes de um arquivo um tem e o outro precisa. A biblioteca padrão de \textit{sockets} POSIX (módulo \texttt{socket} do Python) combinada com \texttt{threading} atua como middleware aqui, de modo a isolar o módulo P2P dos detalhes de baixo nível da comunicação.
...
\begin{itemize}
    ...
    \item \textbf{Assíncrona}: sincronização entre trackers via flooding sobre TCP. O tracker que atualizou seu índice não aguarda confirmação dos demais. Ele dispara o \texttt{SYNC\_TABLE} para cada tracker conhecido em threads paralelas e continua operando, de modo a evitar que a indisponibilidade temporária de um tracker bloqueie os demais.
    
    A convergência é garantida pela reconciliação \textit{anti-entropy} entre trackers, baseada em \textit{digest} de versões periódico (Seção~\ref{sec:rep-protocolo}).
\end{itemize}
...
# arquivo (nome, tamanho, n_chunks) por uma razao simples: sem eles, um
# tracker que conhece o hash apenas via SYNC_TABLE nao consegue responder
# buscas por nome nem aceitar o re-registro pos-download de um peer local.
# Tombstones (ativo: False) os omitem.
#
# O par (origem, seq) identifica a escrita: 'origem' e o tracker que a
# produziu e 'seq' e o seu contador monotonico. O receptor guarda esse
# par junto de cada entrada e mantem o maior seq visto por origem (um
# vetor de versoes), usado para detectar deltas perdidos e dispara-los
# de volta (ver SYNC_DIGEST / SYNC_PULL). O seq so DETECTA perda, o
# desempate de conflito continua sendo LWW por timestamp.
SYNC_TABLE = {
    "type":      "SYNC_TABLE",
    "origem":    "tracker_id",
    "seq":       "int",             # contador monotonico do tracker de origem
...
# Digest de versoes (tracker -> tracker, periodico)
# Em vez de reenviar o estado completo, cada tracker anuncia so o seu
# vetor de versoes: o maior seq que conhece de cada origem (inclusive o
# proprio). O receptor compara componente a componente e, onde o emissor
# estiver a frente, pede os deltas faltantes via SYNC_PULL. 
# Cobre o ponto cego da deteccao inline: quando a ultima escrita de um 
# tracker se perde e ele fica em silencio, nao ha
# seq posterior para revelar a lacuna.
SYNC_DIGEST = {
    "type":    "SYNC_DIGEST",
    "origem":  "tracker_id",
    "versoes": { "tracker_id": "int" }   # maior seq conhecido por origem
}

# Pedido de deltas faltantes (tracker -> tracker)
# Disparado pela deteccao inline (lacuna de seq num SYNC_TABLE), pela
# comparacao de digests, ou pela reintegracao. Pede, para cada origem,
# tudo o que o destinatario originou com seq acima de 'desde_seq'. Esse
# 'desde_seq' e o visto[origem] capturado no instante da deteccao da
# lacuna (fica pendente ate a resposta), nao o visto atual, que pode ja
# ter avancado, pois escritas fora de ordem sao aplicadas na hora. Com
# desde_seq=0 pede o estado inteiro daquela origem, e e assim que um
# tracker reintegrado reconstroi o indice: um SYNC_PULL(desde_seq=0) por
# origem conhecida. A resposta vem como uma ou mais mensagens SYNC_TABLE
# (um evento por seq), na mesma conexao TCP do pedido e incluindo
# tombstones (ativo=False), filtradas do estado atual do destinatario,
# sem log historico, pois cada entrada ja carrega a (origem, seq,
# timestamp) da escrita corrente e o LWW descarta no receptor o que
# estiver velho.
SYNC_PULL = {
    "type":     "SYNC_PULL",
    "faltando": [
        { "origem": "tracker_id", "desde_seq": "int" }
    ]
}
...
# O bootstrap responde com TRACKER_LIST. O indice em
# si o tracker que volta reconstroi sozinho, com SYNC_PULL(desde_seq=0)
# de cada origem conhecida.
TRACKER_REJOIN = {
    "type":       "TRACKER_REJOIN",
    "tracker_id": "string",
    "ip":         "string",
...
# Devolve apenas a lista atual de trackers ativos; nao carrega o indice,
# que e reconstruido a parte via SYNC_PULL(desde_seq=0).
TRACKER_LIST = {
    "type": "TRACKER_LIST",
    "trackers_conhecidos": [
...
Os campos \texttt{query\_id} em \texttt{SEARCH\_FILE}, \texttt{SEARCH\_FORWARD} e \texttt{SEARCH\_RESULT} são UUIDs gerados pelo peer no momento da busca e permitem correlacionar requisição e resposta, de forma a possibilitar que o peer faça múltiplas buscas em paralelo sem confundir os resultados. O \texttt{SEARCH\_FORWARD} carrega ainda o \texttt{origem\_tracker} para que o tracker remoto envie a resposta diretamente ao tracker que originou o roteamento, sem refazer todo o caminho.

\subsubsection*{Sobre a utilidade do \texttt{SEARCH\_FORWARD} com índice replicado}

Como o índice é integralmente replicado entre os trackers (Seção~\ref{sec:replicacao}), em estado estacionário toda busca é respondida localmente e o \texttt{SEARCH\_FORWARD} é dispensável. Ele permanece no protocolo como rede de segurança para a janela transitória de inconsistência: tracker recém-reintegrado que ainda não terminou de reconstruir o índice, ou que perdeu um \texttt{SYNC\_TABLE} e ainda não foi reconciliado pelo \textit{anti-entropy}.

\subsubsection*{Limite de roteamento}
O campo \texttt{ttl} em \texttt{SEARCH\_FILE} é decrementado a cada
\texttt{SEARCH\_FORWARD} entre trackers, de forma a evitar que uma busca percorra a rede indefinidamente. O valor inicial recomendado é 3, suficiente para a topologia de poucos trackers do projeto.

\subsubsection*{Transferência de chunks}
A resposta \texttt{CHUNK\_DATA} é enviada na mesma conexão TCP da requisição. Primeiro o cabeçalho JSON (\texttt{CHUNK\_DATA\_HEADER}, mostrado acima) com o campo \texttt{payload\_bytes} que indica o tamanho, seguido imediatamente pelo payload binário do chunk em si. Esse formato evita o overhead de codificar dados binários (já comprimidos como áudio) em JSON. Antes de iniciar a transferência, o peer cliente envia \texttt{CHUNK\_LIST\_REQUEST} a cada fonte para descobrir quais chunks específicos cada uma possui. Isso permite paralelizar o download entre fontes que tenham chunks complementares.
...
\subsubsection*{Números de sequência e reconciliação entre trackers}
Cada escrita que um tracker origina leva um par \texttt{(origem, seq)}, em que \texttt{seq} é um contador monotônico por tracker. Esse par viaja no \texttt{SYNC\_TABLE} e permite detectar deltas perdidos. O mecanismo completo (vetor de versões, \texttt{SYNC\_DIGEST} periódico e reparo direcionado via \texttt{SYNC\_PULL}) está na Seção~\ref{sec:rep-protocolo}. Vale frisar que o \texttt{seq} apenas \textit{detecta} perda: quem resolve o conflito continua sendo o LWW.
...
\begin{enumerate}
    ...
    \item Se nada encontrado e \texttt{ttl > 0}, encaminha \texttt{SEARCH\_FORWARD} aos demais trackers em paralelo e agrega as respostas recebidas (Seção~\ref{sec:rep-protocolo}).
    \item Tracker retorna \texttt{SEARCH\_RESULT(query\_id, resultados=[\{hash, nome, n\_chunks, peers[\{nome\_peer, ip, porta\}]\}])} ao peer.
    \item Peer envia \texttt{CHUNK\_LIST\_REQUEST(hash)} a cada peer fonte, que responde com \texttt{CHUNK\_LIST(hash, chunks\_disponiveis)}.
    \item Peer monta um plano de download paralelo, de maneira a distribuir os chunks entre as fontes disponíveis, e envia \texttt{CHUNK\_REQUEST(hash, chunk\_index)} para cada chunk. Cada fonte responde com \texttt{CHUNK\_DATA\_HEADER(payload\_bytes)} seguido do payload binário, na mesma conexão TCP.
    \item Após completar o download e validar o SHA-256 do arquivo final, o peer envia \texttt{REGISTER\_FILE(nome\_peer, hash)} ao tracker (re-registro: \texttt{nome}/\texttt{tamanho}/\texttt{n\_chunks} são opcionais), de modo a se tornar também fonte do arquivo.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Cada tracker receptor aplica o \textit{tombstone} por LWW e o retém por 10 minutos antes do descarte definitivo.
\end{enumerate}
...
O \texttt{nome\_peer} satisfaz as propriedades 1. e 2. de um \textit{true identifier}, mas viola estritamente a propriedade 3: nada impede que um usuário desligue sua conta e outro mais tarde se cadastre com o mesmo nome. Para o escopo do projeto isso é aceitável, visto que não há reuso esperado em janelas de tempo curtas.
...
Vale destacar duas decisões:
\begin{enumerate}
    \item a resposta de um \textit{forward} volta na mesma conexão TCP do encaminhamento, sem precisar abrir outra de volta
    \item o tracker que recebe um \texttt{SEARCH\_FORWARD} não reencaminha de novo, o que evita loops sem precisar de nenhuma tabela de roteamento extra
\end{enumerate}
...
O tracker precisa atender múltiplos peers simultaneamente, de forma a receber registros de upload, respondendo buscas e a processar mensagens de sincronização vindas de outros trackers. Um modelo single-threaded bloquearia todas as operações enquanto uma requisição estivesse sendo processada.
...
A implicação é que, ao reiniciar, o tracker começa com as tabelas vazias e reconstrói o índice puxando o estado das demais réplicas ativas, com um \texttt{SYNC\_PULL} desde 0 por origem (Seção~\ref{sec:reintegracao}).
...
Porém, as conexões temporárias de transferência entre peers são tratadas de forma stateless, dado que cada \texttt{CHUNK\_REQUEST} é independente, e a conexão TCP pode ser encerrada e restabelecida sem perda de progresso, pois o gerenciador de chunks controla quais índices já foram recebidos.
...
Em um cenário de implantação, os trackers poderiam rodar em máquinas virtuais ou containers em provedores de nuvem, de forma a permitir escalar o número de trackers conforme a demanda e recriar instâncias automaticamente em caso de falha. Entretanto, dado o escopo do nosso projeto, um esforço dessa magnitude não é viável por questões de tempo de implementação.
...
Para ordenar remoções conflitantes, usaremos timestamps de relógio real, de forma a assumir que os trackers têm NTP sincronizado. Como as atualizações de índice têm granularidade de segundos, a precisão do NTP é mais que suficiente. 
...
Isso evita o seguinte problema: se um tracker estava desconectado e perdeu a mensagem de remoção, ele continuaria a servir um peer que já saiu. Com o tombstone, ao voltar à rede o tracker aplica o LWW normalmente e efetiva a remoção. O período de retenção é de \textbf{10 minutos}, escolhido para cobrir partições curtas e ficar acima do intervalo de reconciliação \textit{anti-entropy} entre trackers (Seção~\ref{sec:rep-protocolo}), de modo que uma remoção perdida seja reposta antes de expirar. Passado esse prazo, o registro é descartado de vez. 
...
Um novo tracker é iniciado com o endereço de pelo menos um tracker já ativo (\textit{bootstrap node}). Ele se anuncia a esse nó com \texttt{TRACKER\_REJOIN} e recebe de volta apenas a lista de trackers ativos (\texttt{TRACKER\_LIST}), o índice em si é reconstruído separadamente via \texttt{SYNC\_PULL(desde\_seq=0)} por origem. O \textit{bootstrap} divulga sua existência aos demais via \texttt{TRACKER\_ANNOUNCE}. A partir daí o novo tracker está integrado e passa a receber as atualizações.

\subsubsection*{Reconciliação anti-entropy}

O \texttt{SYNC\_TABLE} é \textbf{incremental} (carrega só o que mudou) e enviado em modo \textit{best-effort}: o remetente não retransmite em caso de falha. Isso levanta uma pergunta: se um tracker perder uma única mensagem \texttt{SYNC\_TABLE} por uma falha transitória de TCP, mas continuar no ar, esse delta sumiria do caminho de \textit{flooding} e ele ficaria desatualizado. O \texttt{SYNC\_TABLE} sozinho, portanto, \textbf{não} fecha o problema de entropia.

A convergência é garantida por dois mecanismos complementares que trabalham sobre um \textbf{número de sequência por origem}.

Números de sequência. Cada tracker mantém um contador monotônico \texttt{meu\_seq} e carimba toda escrita que origina (registro ou \textit{tombstone}) com o par \texttt{(origem, seq)}. Esse par viaja no \texttt{SYNC\_TABLE} e é guardado junto de cada entrada do índice, ao lado do \textit{timestamp}. Cada tracker mantém ainda um \textbf{vetor de versões} \texttt{visto[origem]}, com o maior \texttt{seq} já aplicado de cada origem, um inteiro por tracker. É importante separar os papéis: o \texttt{seq} serve só para \textbf{detectar} que algo se perdeu, quem \textbf{vence} um conflito continua a ser decidido pelo LWW (\textit{timestamp}, com \texttt{tracker\_id} de desempate).

Detecção inline e reparo direcionado. Como a topologia é totalmente conectada e o TCP preserva a ordem dentro de cada conexão, em operação normal os \texttt{seq} de um mesmo tracker chegam em sequência. Quando um \texttt{SYNC\_TABLE} de \texttt{X} chega com \texttt{seq} maior que \texttt{visto[X]\,+\,1}, o receptor sabe na hora que perdeu um delta \textit{daquele} tracker e pede só o que falta com um \texttt{SYNC\_PULL(\{X, visto[X]\})}. O tracker à frente responde filtrando o \textit{estado atual} pelas entradas que originou com \texttt{seq} acima do pedido e as reenvia como \texttt{SYNC\_TABLE}. Não é preciso manter um \textit{log} histórico de deltas: como cada entrada já carrega a \texttt{(origem, seq, timestamp)} da escrita corrente, reenviar o estado atual basta, e o LWW no receptor descarta o que estiver velho. O reparo custa o tamanho do que \textit{de fato} divergiu.

\subsubsection*{Estado de pendência}
 O \texttt{visto[origem]} é o \textit{maior} \texttt{seq} já visto daquela origem, não uma garantia de que tudo abaixo dele chegou: ele avança por \texttt{max} mesmo com uma lacuna em aberto. Receber a escrita 11 com \texttt{visto[X]\,=\,9} aplica a 11 no índice na hora (por LWW, como toda escrita) e leva \texttt{visto[X]} a 11, sem esperar a 10. A lacuna não trava o contador, ela vira uma \textbf{pendência}: um pequeno conjunto de pares \texttt{(origem, desde\_seq)} em aberto, em que \texttt{desde\_seq} guarda o \texttt{visto[X]} do instante em que o buraco foi detectado (9), e não o valor já avançado. É esse \texttt{desde\_seq} fixado que vai no \texttt{SYNC\_PULL}, de modo que o pedido cubra a 10 mesmo que \texttt{visto[X]} já esteja em 15. As escritas 12 a 15, que chegam depois, são aplicadas normalmente e não abrem novas pendências (cada uma é contígua com a anterior). A pendência governa apenas o que o pull pede e se fecha quando a resposta chega. Como o reparo é idempotente, ela é re-disparável: se a própria resposta se perder, a próxima escrita de \texttt{X} (detecção inline) ou o \texttt{SYNC\_DIGEST} periódico reabre o pedido.

\subsubsection*{Digest de versões periódico}
A detecção inline tem um ponto cego: se a \textit{última} escrita de um tracker se perde e ele fica em silêncio, não vem um \texttt{seq} posterior para revelar a lacuna. Para fechar essa borda, cada tracker envia periodicamente, num intervalo folgado de \textbf{5 minutos}, uma mensagem \texttt{SYNC\_DIGEST} com apenas o seu vetor de versões. O receptor compara componente a componente e, onde o emissor estiver à frente, dispara o mesmo \texttt{SYNC\_PULL}. Como o digest tem um custo computacional baixo, pode rodar com folga e ainda assim repor qualquer delta perdido antes de um \textit{tombstone} expirar, contanto que o intervalo do digest seja mantido abaixo da retenção dos \textit{tombstones} (Seção~\ref{sec:modelo_consistencia}).

É por isso que, quando um envio de \texttt{SYNC\_TABLE} falha, o remetente apenas marca o destino como suspeito e não reenvia na hora: o próprio destino, ao notar a lacuna (inline ou pelo digest), puxa o que faltou.

Em complemento, no sentido peer$\rightarrow$tracker, o \texttt{SEED\_REPORT} periódico (Seção~\ref{sec:mensagens}) reconcilia o índice \textit{local} do tracker ao qual o peer se reporta: hashes ausentes no relatório viram \textit{tombstone}, novos são registrados. E um tracker que tenha ficado muito tempo fora, ao voltar, recupera todo o índice de uma réplica viva com um \texttt{SYNC\_PULL} desde 0 por origem (que também inicializa seu vetor de versões).
...
Ao iniciar, o tracker envia uma mensagem \texttt{TRACKER\_REJOIN} ao \textit{bootstrap node}, que responde com a lista atual de trackers ativos (\texttt{TRACKER\_LIST}) e propaga \texttt{TRACKER\_ANNOUNCE} aos demais, que adicionam o tracker reintegrado às suas listas de trackers ativos. Com a lista de trackers, o tracker reconstrói o índice como um caso particular do reparo comum: emite um \texttt{SYNC\_PULL} com \texttt{desde\_seq=0} para cada origem conhecida e recebe de volta o estado atual (inclusive \textit{tombstones}) como mensagens \texttt{SYNC\_TABLE}, ao aplicá-lo por LWW, inicializa seu vetor de versões e seu próprio contador de sequência. Em seguida, cada tracker ativo cede uma fração de seus peers ao tracker reintegrado, de maneira a enviar a cada peer cedido uma mensagem \texttt{REASSIGN\_TRACKER} com o endereço do novo tracker. A Figura~\ref{fig:reintegracao} detalha essa sequência no tempo.
...
O critério de balanceamento é direto. Cada tracker ativo divide seu número atual de peers pelo número total de trackers na rede (que inclui o reintegrado) e cede o excedente, o que distribui a carga de forma aproximadamente uniforme sem necessidade de coordenação centralizada.
...
A lista de trackers que cada peer mantém localmente é estática no código apresentado. Para suportar a adição dinâmica de trackers, a lista pode ser estendida por uma resposta opcional do tracker em qualquer comunicação subsequente, o que inclui um campo \texttt{trackers\_conhecidos} com o conjunto atual de trackers ativos. O peer atualiza sua lista local com essa informação. Para simplicidade do protótipo, no escopo deste trabalho assumimos a lista estática, mas a limitação é registrada aqui explicitamente.

\subsection{Disponibilidade ou Confiabilidade?}\label{sec:disp_conf}

Entre disponibilidade (estar pronto para uso a qualquer instante) e confiabilidade (operar continuamente, sem falhas, por longos períodos), o PeerSpot prioriza a \textbf{disponibilidade}. Essa decisão vem do próprio domínio, pois, em um serviço de compartilhamento de música, é melhor que uma busca responda rápido com um resultado talvez desatualizado (por exemplo, um peer que acabou de sair) do que travar a resposta à espera de certeza absoluta.

Essa preferência é coerente com as escolhas que já fizemos: o índice é replicado em todos os trackers (qualquer um responde), o peer troca na hora para um tracker de backup quando o principal cai (Seção~\ref{sec:fallback_peer}) e o sistema opera sob consistência eventual (Seção~\ref{sec:modelo_consistencia}). Em troca, abrimos mão da confiabilidade estrita, de forma a aceitar pequenas inconsistências transitórias em favor da disponibilidade e da simplicidade.

\subsection{Tipos de Falha que se Deseja Tolerar}\label{sec:tipos-falha}

O PeerSpot assume o modelo \textit{crash} (\textit{fail-stop}): os nós podem parar, mas não passam a se comportar de forma arbitrária. A seguir, percorremos os tipos de falha:
...
\begin{itemize}
    ...
    \item \textbf{Omissão (perda de mensagens):} \textbf{mitigada}. Mensagens perdidas são repostas pela reconciliação \textit{anti-entropy} entre trackers (detecção por número de sequência e reparo por \texttt{SYNC\_PULL}, mais o \texttt{SYNC\_DIGEST} periódico), pelo \texttt{SEED\_REPORT} no sentido peer$\rightarrow$tracker e pela reconstrução do índice na reintegração (\texttt{SYNC\_PULL} desde 0). Além disso, o uso de TCP já evita a maior parte das perdas.
...
\begin{itemize}
    \item \textbf{Trackers:} como o índice está replicado por inteiro nos $N$ trackers, a busca continua a funcionar enquanto pelo menos um deles estiver no ar, ou seja, toleramos a queda de até $N-1$ trackers ao mesmo tempo. Os peers migram sozinhos para o próximo tracker da lista (Seção~\ref{sec:fallback_peer}).
...
A detecção é sempre baseada em \textbf{timeout}, seja sobre uma comunicação periódica, seja sobre uma conexão. O caso que realmente exige detecção ativa é a \textbf{saída desordenada} de um peer (queda de conexão, processo encerrado de repente, máquina desligada), em que não chega nenhum \texttt{PEER\_LEAVE} que avisa da saída. No PeerSpot, há três caminhos de detecção:
...
\begin{enumerate}
    ...
    \item \textbf{Recuperação por transferência de estado:} o \texttt{SYNC\_PULL} repõe deltas perdidos em regime e reconstrói o índice de um tracker reintegrado (pedido desde 0) (Seção~\ref{sec:reintegracao}).
\end{enumerate}
...
O PeerSpot é, por escolha, um sistema \textbf{AP}: diante de uma partição entre trackers, cada lado continua a responder a buscas e a aceitar registros com seu índice local (prioriza A), abrindo mão da consistência forte (sacrifica C) e operando sob consistência eventual. Quando a partição é curada, os índices voltam a convergir via \textit{anti-entropy} entre trackers (\textit{digest} de versões e \texttt{SYNC\_PULL}) e via \texttt{SEED\_REPORT}, com os conflitos resolvidos por LWW (Seção~\ref{sec:rep-protocolo}).
...
\begin{itemize}
    ...
    \item \textbf{Tracker que retorna:} reconstrói todo o índice a partir de um tracker ativo, via \texttt{TRACKER\_REJOIN} seguido de um \texttt{SYNC\_PULL} desde 0 por origem (Seção~\ref{sec:reintegracao}). É como restaurar o estado atual de uma réplica viva, sem precisar reaplicar um log de operações. Em seguida vem o rebalanceamento de carga.
    \item \textbf{Partição curta:} a reconciliação \textit{anti-entropy} entre trackers (\textit{digest} de versões + \texttt{SYNC\_PULL}) repõe sozinha as entradas que faltaram.
